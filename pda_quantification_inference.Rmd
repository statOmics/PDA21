...
```{r}
t.test(intensity ~ genotype, data = WP_003023392)
```
...
- When we repeat the experiment, the probability to observe a fold change more extreme than a `r format(2^lmHlp$coef[2],digits=3)` fold ($\log_2 FC=`r format(lmHlp$coef[2],digits=3)`$) down or up regulation by random change (if $H_0$ is true) is 3 out of 100.000.  
