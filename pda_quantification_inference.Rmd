...
```{r}
peptidesFile <- "https://raw.githubusercontent.com/statOmics/PDA20/data/quantification/francisella/peptides.txt"
```
...
- Consider testing DA for all $m=`r nrow(pe[["protein"]])`$ proteins simultaneously
- What if we assess each individual test at level $\alpha$?
$\rightarrow$ Probability to have a false positive among all m simultatenous
test $>>>  \alpha= 0.05$
...
When we want to infer on differential abundance of $m$ multiple proteins we have to address the multiple testing issue. 
...
FDP = \frac{FP}{R}
\]
However, this quantity cannot be observed because in practice we only know $R$ but we do not know $FP$.
...
- Suppose that a researcher rejects all null hypotheses for which $p < 0.001$. 

- If we use $p < 0.001$, we expect $0.001 \times m_0$ tests to return false positives. A conservative estimate of the number of false positives that we can expect can be obtained by considering that the null hypotheses are true for all features, $m_0 = m =  10000$. We then would expect $0.001 \times 10,000 = 10$ false positives ($FP=10$).
...
```{r}
gridExtra::grid.arrange(volcanoT +    xlim(-3,3)
,volcano +     xlim(-3,3)
,nrow=2)
```
