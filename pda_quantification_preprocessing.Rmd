...
$$y_{ip}^\text{norm} = y_{ip} - \mu_i$$ 
with $\mu_i$ the median intensity over all observed peptides in sample $i$.
...
- Our R/Bioconductor package `msqrob2` can be used with a GUI/shinyApp in the `msqrob2gui` package or in R markdown scripts. 
